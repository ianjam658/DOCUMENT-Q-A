...
- **Python version**: Render does **not** use Heroku's `runtime.txt` convention. It reads a `.python-version` file (just the bare version number, e.g. `3.11.9`, no `python-` prefix) or a `PYTHON_VERSION` environment variable. This repo sets both — the file for local/other-platform use, and the env var in `render.yaml` as a fallback. If you deployed manually (not via Blueprint), you'll need to add `PYTHON_VERSION=3.11.9` under Environment in the Render dashboard yourself, since manual services don't read `render.yaml`. Without this, Render may build against a very new Python release before packages like `psycopg2-binary` have published compatible prebuilt wheels for it, causing an `ImportError: undefined symbol` crash at startup that has nothing to do with your code.
